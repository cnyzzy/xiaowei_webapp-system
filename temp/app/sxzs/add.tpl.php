...
<?php if(!empty($Array['type'])) { ?><?php echo $Array['type'];?><?php } else { ?>-<?php } ?>						</button>
	...
	<li class="mui-table-view-cell">
				...
				<div class="mui-card-content">
					 <input id="rq"  type="hidden" value="">
					 <input id="sj"  type="hidden" value="">
					 <input id="sc"  type="hidden" value="">
					<div class="mui-card-content-inner">
						<h5 class="mui-content-padded">预约日期</h5>
				<button id='rqxz'  class="btn mui-btn mui-btn-block">选择日期 ...</button>
				<h5 class="mui-content-padded">预约时间</h5>
				<button id='sjxz'  class="btn mui-btn mui-btn-block">选择时间 ...</button>
				<h5 class="mui-content-padded">预约时长</h5>
				<button id='scxz'  class="btn mui-btn mui-btn-block">选择时长 ...</button>
									 <input id="postrq"   type="hidden" value="">
					 <input id="postsj"  type="hidden" value="">
        ...
        if(toast.length>0){
			...
			(function($) {
				$('.mui-scroll-wrapper').scroll({
					...
					document.getElementById('rqxz').addEventListener("tap",function(){
//           
                var dtpicker = new mui.DtPicker({
                    type: "date",//设置日历初始视图模式    //真正的月份比写的多一个月。  type的类型你还是可以选择date, datetime month time  hour 
                    beginDate: new Date(<?php ECHO(date('Y',$timeee))?>, <?php ECHO(date('m',$timeee))?>, <?php ECHO(date('j',$timeee))?>),//设置开始日期   
                    endDate:  new Date(<?php ECHO(date('Y',$timee2))?>, <?php ECHO(date('m',$timee2))?>, <?php ECHO(date('j',$timee2))?>),//设置结束日期    //真正的是10.21
...
document.getElementById("rq").value=e.y.text+"-"+e.m.text+"-"+e.d.text;
document.getElementById("rqxz").innerText=document.getElementById("rq").value;
                })
    });    
    					document.getElementById('sjxz').addEventListener("tap",function(){
//           
                var dtpicker = new mui.DtPicker({
                    type: "time",//设置日历初始视图模式    //真正的月份比写的多一个月。  type的类型你还是可以选择date, datetime month time  hour 
                    beginDate:  new Date(<?php ECHO(date('Y',$timeee))?>, <?php ECHO(date('m',$timeee))?>, <?php ECHO(date('j',$timeee))?>, <?php ECHO(date('G',$Array['opentime']))?>, <?php ECHO(date('i',$Array['opentime']))?>),//设置开始日期   
                    endDate: new Date(<?php ECHO(date('Y',$timeee))?>, <?php ECHO(date('m',$timeee))?>, <?php ECHO(date('j',$timeee))?>, <?php ECHO(date('G',$Array['endtime']))?>, <?php ECHO(date('i',$Array['endtime']))?>),//设置结束日期    //真正的是10.21
                  
                
                })
                dtpicker.show(function (e) {document.getElementById("sj").value=e.h.text+":"+e.i.text;
                document.getElementById("sjxz").innerText=document.getElementById("sj").value;
...
document.getElementById("sc").value=e.h.text+"小时"+e.i.text+"分钟";
	...
	var postnumber=10;
			var postrq=document.getElementById("rq").value;
			var postsj=document.getElementById("postrq").value+" "+document.getElementById("sj").value;
  ...
  mui.openWindow({
    ...
    url:'detail.html'
		...
		mui('body').on('tap', '.mui-popover-action li>a', function() {
			...
			document.getElementById("postrq").value=document.getElementById("rq").value;
			document.getElementById("postsj").value=document.getElementById("postrq").value+" "+document.getElementById("sj").value;
  check = true;
...
if(!document.getElementById("postrq").value || document.getElementById("postrq").value.trim() == "") {
    mui.alert("日期不允许为空");
    check = false;
    return false;
}
if(!document.getElementById("postsj").value || document.getElementById("postsj").value.trim() == "") {
    mui.alert("时间不允许为空");
    check = false;
    return false;
}
