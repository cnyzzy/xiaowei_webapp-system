...
<div class="title">
	<p>亲爱的<?php echo $_SESSION['zid']['name'];?>,感谢参加今天的活动</p>
	<P>当前提交的数据属于：<?php echo $timee;?><br>每日提交时间为当日早上六点至八点。
</div>
</div>
  ...
  <a href="javascript:void(0);" onclick="uploadphoto.click()" class="uploadbtn">点击上传文件</a>
		...
		<article id="tip">
			<div class="pack">
				<h1 class="tc">提示</h1>
				<p class="tc"></p>
				<a id='aa' href="<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/zcqup">确定</a>
...
$(document).ready(function(e) {
	$(window).on("load", function() {
				...
				top.location.href = "<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/zcqup"; 
				ttt=$("#tip .pack p").html(); 
					...
					if(ttt=="您今天的数据已经提交!"||ttt=="上传成功，请等待审核"||ttt=="未知原因，提交失败，请重新尝试")
					{
						...
						$("#tip .pack a").fadeOut();
					top.location.href = "<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/zcq"; 
...
alert('请填写步数!')
return false;}
$.post("<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/zcqok",{
         step:  ssss,
   ...
   $('#uploadphoto').localResizeIMG({
      ...
      success: function (result) {  
		...
		$.ajax({
		   type: "POST",
		   url: "<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/zcqupload",
		   data: submitData,
		   dataType:"json",
