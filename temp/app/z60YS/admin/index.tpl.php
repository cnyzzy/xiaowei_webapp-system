...
$.ajax({
		type:"POST",
		url:"<?php echo $arrInfo['url'];?>/admin/Z60ys/do",
...
function isokok(id,isok){
 if(confirm("确定要修改文章的显示状态么？")){
    $.post('<?php echo $arrInfo['url'];?>/admin/Z60ys/do',{'do':'isopen','id':id,'isok':isok},function(rs){
        if(rs==1){
            window.location.reload(true);
        }
    })}
...
function add(){
 if(confirm("请谨慎操作")){
           window.open('<?php echo $arrInfo['url'];?>/admin/Z60ys/add/');
        }
    }
function edit(id,gID){
 if(confirm("请谨慎操作"+id+gID)){
       if (gID=='1'||gID=='2') window.open('<?php echo $arrInfo['url'];?>/admin/Z60ys/edit/'+id);
	   if (gID=='3'||gID=='4') window.open('<?php echo $arrInfo['url'];?>/admin/Z60ys/imgedit/'+id);
	   if (gID=='5'||gID=='6') window.open('<?php echo $arrInfo['url'];?>/admin/Z60ys/vedit/'+id);
        ...
        <div class="tpl-content-wrapper">
            ...
            <ol class="am-breadcrumb">
                ...
                <li><a>APP管理</a></li>
                <li class="am-active">校庆文章管理</li>
            </ol>
            <div class="tpl-portlet-components">
                <div class="portlet-title">
                    <div class="caption font-green bold">
                        <span class="am-icon-code"></span> 校庆文章管理
                    </div>
                    ...
                    <div class="am-g">
                        <div class="am-u-sm-12">
						  <div class="am-btn-toolbar">
                               ...
                               <a href="<?php echo $arrInfo['url'];?>/admin/Z60ys/add"> <button type="button" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span> 新建文章</button></a>
                             <a href="<?php echo $arrInfo['url'];?>/admin/Z60ys/imgadd"> <button type="button" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span> 新建相册</button></a>
							    <a href="<?php echo $arrInfo['url'];?>/admin/Z60ys/vadd"> <button type="button" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span> 新建视频</button></a>
                                ...
                                <table class="am-table am-table-striped am-table-hover table-main">
									...
									<?php foreach((array)$Array as $key=>$loopChild) {?>
                                        <tr>                                          
                                            ...
                                            <td class="am-hide-sm-only"> <?php if($loopChild['groupid']=='1') { ?>简介<?php } ?><?php if($loopChild['groupid']=='2') { ?>历史<?php } ?><?php if($loopChild['groupid']=='3') { ?>校园风光<?php } ?><?php if($loopChild['groupid']=='4') { ?>记忆光影<?php } ?><?php if($loopChild['groupid']=='5') { ?>视频<?php } ?><?php if($loopChild['groupid']=='6') { ?>校友<?php } ?></td>
                                            ...
                                            <td>
                                                <div class="am-btn-toolbar">
                                                    <div class="am-btn-group am-btn-group-xs">
                                                        <button onclick="edit('<?php echo $loopChild['id'];?>','<?php echo $loopChild['groupid'];?>');" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span> 编辑</button>
