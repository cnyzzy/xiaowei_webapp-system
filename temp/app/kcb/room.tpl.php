...
<html lang="en">
    <head>
        ...
        <title>教室课程表-<?php echo $arrInfo['name'];?></title>
        ...
        <script>(function (doc, win) {
		 ...
		 function login(form) {
				...
				$.ajax({  
          type:"POST", 
		url:"<?php echo $arrInfo['url'];?>/kcb/do/room",
		 async:true, 
		 timeout : 5000,
		 data:"&isis=1&r=<?php echo(urlencode($roomm))?>",
        ...
        <header>
         ...
         <div class="titlebar reverse">
             <a href="javascript:history.go(-1);">
                ...
                <h1><?php echo $roomm;?>教室课程表</h1>
            </div>
        </header>
        <article style="bottom: 0;">
		                  <div class="weui_search_bar" id="search_bar" style="z-index:0;">
			...
			<tr>
				<td><?php if(!EMPTY($Child['xy'])) { ?><?php echo $Child['xy'];?><?php } else { ?>[无数据]<?php } ?></td>
					<td><?php if(!EMPTY($Child['xzb'])) { ?><?php echo $Child['xzb'];?><?php } else { ?>[无数据]<?php } ?></td>
											<td><?php if(!EMPTY($Child['coursename'])) { ?><a href="<?php echo $arrInfo['url'];?>/kcb/c/<?php echo(urlencode($Child['coursename']))?>"><?php echo $Child['coursename'];?></a><?php } else { ?>[无数据]<?php } ?></td>
