    ...
    <body ontouchstart>
      ...
      <?php if(isset($arrtop)) { ?>	 
		...
		<a style="color:#000" class="weui_cell">
          <div id="no<?php echo $loopChild['id'];?>" oid="<?php echo $loopChild['id'];?>" <?php if(!empty($loopChild['sid'])) { ?>sid="<?php echo $loopChild['sid'];?>"<?php } ?> <?php if(!empty($loopChild['isok'])) { ?>top="1"<?php } ?> class="weui-media-box weui-media-box_text" style="border-top: 4.5px solid <?php echo($colorarray[array_rand ($colorarray, 1)]);?>;">
	  ...
	  <li class="weui-media-box__info__meta2 weui-media-box__info__meta_extra"><?php if(!empty($loopChild['isok'])) { ?><?php if($loopChild['isok']=='1') { ?>已通过<?php } ?><?php if($loopChild['isok']=='2') { ?>不予通过<?php } ?><?php } else { ?>待审核<?php } ?><?php if(!empty($loopChild['sid'])) { ?> #<?php echo $loopChild['sid'];?><?php } else { ?> <?php } ?></li>            </ul>
          </div></a>
...
$.toast("未通过审核","cancel");			  
 ...
 $.showLoading();
 window.location.href="<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/more/"+sid;}
              }
            },
            {
              text: "删除",
 	  ...
 	  $.ajax({
		...
		url:"<?php echo $arrInfo['url'];?>/shiji/do/unmyid",timeout : 5000,
		data:"&id="+oid,
		beforeSend:function(){  
$.showLoading("删除中");
...
*/
  ...
  <script>
     ...
     $(document.body).infinite().on("infinite", function() {
	  ...
	  $.ajax({
                url: '<?php echo $arrInfo['url'];?>/<?php echo $AppName;?>/do/my',
                dataType: 'json',
                type: 'POST',
				data:"&id="+topnum,
				timeout : 5000,
