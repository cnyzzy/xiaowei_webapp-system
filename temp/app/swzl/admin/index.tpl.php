...
$.ajax({
		type:"POST",
		url:"<?php echo $arrInfo['url'];?>/admin/swzl/do",
...
function isokok(id,isok){
 if(confirm("确定要修改审核状态么？")){
    $.post('<?php echo $arrInfo['url'];?>/admin/swzl/do',{'do':'isopen','id':id,'isok':isok},function(rs){
        if(rs==1){
...
function edit(id){
 if(confirm("请谨慎操作")){
           window.open('<?php echo $arrInfo['url'];?>/admin/swzl/edit/'+id);
        }
    }
                            ...
                            <div class="am-btn-toolbar">
                                <div class="am-btn-group am-btn-group-xs">
                                  <?php if($bbb!='index') { ?> <a href="<?php echo $arrInfo['url'];?>/admin/walking"> <button type="button" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span> 全部数据</button></a><?php } ?>
                                    <a href="<?php echo $arrInfo['url'];?>/admin/walking/excel/<?php echo $bbb;?>/<?php echo $ccc;?>"><button type="button" class="am-btn am-btn-default am-btn-secondary"><span class="am-icon-save"></span> 保存表格</button></a>
                                </div>
                            </div>
                        </div>
                                ...
                                <table class="am-table am-table-striped am-table-hover table-main">
									...
									<?php foreach((array)$Array as $key=>$loopChild) {?>
                                        <tr>                                          
                                            ...
                                            <td><a href="<?php echo $arrInfo['url'];?>/admin/swzl/<?php echo $aaa;?>/title/<?php echo $loopChild['title'];?>"><?php echo $loopChild['title'];?></a></td>
                                            <td class="am-hide-sm-only"> <a href="<?php echo $arrInfo['url'];?>/admin/swzl/<?php echo $aaa;?>/tname/<?php echo $loopChild['tname'];?>"><?php echo $loopChild['tname'];?></a></td>
                                            ...
                                            <td><a href="<?php echo $arrInfo['url'];?>/admin/swzl/<?php echo $aaa;?>/mobile/<?php echo $loopChild['data'];?>"><?php echo $loopChild['mobile'];?></a></td>
											<td class="am-hide-sm-only"><?php ECHO(date('m-j G:i:s',$loopChild['addtime']))?></td>
											 <td><?php echo $loopChild['gx'];?></td>
											  <td><a href="<?php echo $arrInfo['url'];?>/admin/swzl/pl/sid/<?php echo $loopChild['id'];?>"><?php echo $loopChild['pl'];?></a></td>
											   <td><?php echo $loopChild['ll'];?></td>
                                            <td>
                                                <div class="am-btn-toolbar">
                                                    <div class="am-btn-group am-btn-group-xs">
                                                        ...
                                                        <button onclick="isokok('<?php echo $loopChild['id'];?>','<?php if($loopChild['isok']=='0') { ?>1<?php } ?><?php if($loopChild['isok']=='1') { ?>2<?php } ?><?php if($loopChild['isok']=='9') { ?>0<?php } ?><?php if($loopChild['isok']=='2') { ?>9<?php } ?>');"  class="am-btn am-btn-default am-btn-xs am-hide-sm-only"><span class="am-icon-copy"></span> <?php if($loopChild['isok']=='0') { ?>通过<?php } ?><?php if($loopChild['isok']=='1') { ?>完结<?php } ?><?php if($loopChild['isok']=='2') { ?>隐藏<?php } ?><?php if($loopChild['isok']=='9') { ?>未审<?php } ?></button>
