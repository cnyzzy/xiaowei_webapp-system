...
$arrQquniapi = SetRead('/system/config/Public/Qquniapi.php');
  ...
  'pertime' => 0,
    ...
    $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".$arrQquniapi['APPID']."&secret=".$arrQquniapi['APPKEY'];
    $res = $this->getJson($url);
...
IF(!$atoken['time']||!$atoken['pertime']){
  ...
  'pertime' => 0,
    ...
    $url = "https://api.uni.qq.com/cgi-bin/token_v2?appid=".$arrQquniapi['APPID']."&secret=".$arrQquniapi['APPKEY'];
    $res = $this->getJson($url);
	if($res['errcode']!='0')
	{
		 return $res;
