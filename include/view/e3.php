...
{  
    ...
    public function responseMsg(){  
	    ...
	    $sql = "SELECT  * FROM wxid WHERE openid ='{$fromUsername}'";
                    ...
                    if($MsgType == 'text'){  
                        ...
                        if(!empty( $content )){  
                            ...
                            if($content == "课表"||$content == "课程表"){  
                                ...
                                $resultStr = sprintf($newsTpl, $fromUsername, $toUsername, $time, 'news', $ArticleCount,'点击查看课程表','','', 'http://weixin.yctu.edu.cn/kcb','使用更多功能','描述2','http://weixin.yctu.edu.cn/app/home/model/images/yes.png','http://weixin.yctu.edu.cn');  
                                echo $resultStr;  
                                $this->log($resultStr);  
                            }else{  
                                //回复文本信息  
                    ...
                    //如果是地理位置  
                        ...
                        $Label = $postObj->Label;//地里位置信息  
                          //回复图文消息,ArticleCount图文消息个数,多条图文消息信息，默认第一个item为大图  
                                ...
                                $resultStr = sprintf($newsTpl, $fromUsername, $toUsername, $time, 'news', $ArticleCount,$Label,'查看如何前往此处','', 'http://apis.map.qq.com/uri/v1/routeplan?type=bus&from=盐城师范学院&tooord='.$Location_X.','.$Location_Y.'&to='.$Label.'&policy=1&referer=YCTU-1958','查看如何前往学校','BY:ZY','http://weixin.yctu.edu.cn/app/home/model/images/map.png','http://apis.map.qq.com/uri/v1/routeplan?type=bus&fromcoord='.$Location_X.','.$Location_Y.'&to=盐城师范学院&policy=1&referer=YCTU-1958');  
                                echo $resultStr;  
                                $this->log($resultStr);  
