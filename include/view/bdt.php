<?php
define('RROOT',ZRoot.'/system/data/app/home/tlogin/');
...
//解除绑定
 ...
 $sql = "SELECT * FROM wbid WHERE number ='{$number}' and isok=1";
		$result = $DB->query($sql);
...
$sql = "update wbid SET isok = 0 ,deletetime = '{$time}'  where number = '{$number}' and isok = 1" ;

	if($DB->query($sql))
	{
		if($_SESSION['my']=='WB')unset($_SESSION['zid']);
ECHO"1";
			}else{ECHO"0";}	
}else{
	if($_SESSION['my']=='WB')unset($_SESSION['zid']);
...
ErrorMsg($Errormsg,'点击返回>>','/home/qlogin/1');}
$_SESSION['res']=$res;
...
$name=substr($xm[1][0],0,-6); 
 $sql = "SELECT * FROM wbid WHERE openid ='{$_SESSION['wx']['openid']}' and isok=1";
		...
		$row = $DB->fetch_array($result);
			if(empty($row)){
				//写入绑定
		$sql11 = "SELECT * FROM wbid WHERE number ='{$username}' and isok=1";
...
$sql3 = "insert into jwinfo ($strk) values ($strv)"; 
 	$DB->query($sql3);
			 ...
			 $sql = "INSERT INTO `wbid` ( `type`, `number`, `name`, `openid`, `nickname`, `img`,  `addtime`, `deletetime`, `isok`, `islock`, `stop`) VALUES ( 1, '{$username}', '{$name}', '{$openid}', '{$nickname}', '{$img}', '{$time}', NULL, 1, 0, 0);";
			if(!$DB->query($sql)){
				$Errormsg=array (
  'error_type' => '提示',
  'msg' => '数据库故障',
...
ErrorMsg($Errormsg,'点击返回>>','/home/qlogin/1');}else{
	$_SESSION['zid']=$row;
	}
				}
			}
...
$sql2 = "SELECT * FROM ghzl WHERE name ='{$username}' and number ='{$number}'";
  ...
  'msg' => '该姓名和工号不对应，请检查输入或联系管理员',
		...
		$sql22 = "SELECT * FROM wbid WHERE number ='{$number}' and isok=1";
...
ErrorMsg($Errormsg,'点击返回>>','/home/qlogin/2');
				}else{
					//已经绑定过
						if($row['stop']==1){
							$Errormsg=array (
...
ErrorMsg($Errormsg,'点击返回>>','/home/qlogin/2');}else{
	$_SESSION['zid']=$row;
	}
				}
	}
			...
			if(empty($row)){
			 ...
			 $sql = "INSERT INTO `wbid` ( `type`, `number`, `name`, `openid`, `nickname`, `img`,  `addtime`, `deletetime`, `isok`, `islock`, `stop`) VALUES ( 2, '{$number}', '{$row2['name']}', '{$openid}', '{$nickname}', '{$img}', '{$time}', NULL, 1, 0, 0);";
			if(!$DB->query($sql)){
				$Errormsg=array (
  'error_type' => '提示',
  'msg' => '数据库故障',
...
//访客
	...
	{ 
		...
		$row = $DB->fetch_array($result);
			if(empty($row)){
				//写入绑定
		$sql11 = "SELECT * FROM wbid WHERE number ='{$number}' and isok=1";
		$result11 = $DB->query($sql11);
		$row11 = $DB->fetch_array($result11);
		if($row11['stop']==1){
			$Errormsg=array (
...
ErrorMsg($Errormsg);
			 ...
			 $sql = "INSERT INTO `wbid` ( `type`, `number`, `name`, `openid`, `nickname`, `img`,  `addtime`, `deletetime`, `isok`, `islock`, `stop`) VALUES ( 3, '{$number}', '{$username}', '{$openid}', '{$nickname}', '{$img}', '{$time}', NULL, 1, 0, 0);";
			if(!$DB->query($sql)){
				$Errormsg=array (
  'error_type' => '提示',
  'msg' => '数据库故障',
