...
$NOW = empty($params[1]) ? 'all':($params[1]);
ob_end_clean();
error_reporting(0);
header('Content-Type: application/vnd.ms-excel;charset=gb2312');
if($NOW=='all')
{$name="现场抽奖到场总表";
$sql = "SELECT * FROM cjhd_on order by addtime desc";
}else{

$name="".$NOW."现场抽奖到场";
 $sql = "SELECT * FROM cjhd_on where sid='".$NOW."' order by addtime desc";
}
...
echo   $loopChild['ip']."\t"; 
if( $loopChild['isok']=='1')echo(iconv('UTF-8', 'GBK',  "可用")."\t";
if( $loopChild['isok']=='0')echo(iconv('UTF-8', 'GBK',  "忽略")."\t";
