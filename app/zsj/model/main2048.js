...
function prepareForMobile(){
    ...
    $('.grid-container').css('padding',cellSpace);
    $('.grid-container').css('border-radius',0.02*gridContainerWidth);
        $('.game-container').css('width',gridContainerWidth - 2*cellSpace);
    $('.game-container').css('height',gridContainerWidth - 2*cellSpace);
    $('.game-container').css('padding',cellSpace);
    $('.game-container').css('border-radius',0.02*gridContainerWidth);
...
$(document).keydown(function (event) {
  ...
  //touchstart事件  
			...
			var grid = document.getElementById("grid-container")
                grid.addEventListener('touchstart', touchSatrtFunc, false);  
               grid.addEventListener('touchmove', touchMoveFunc, false);  
                grid.addEventListener('touchend', touchEndFunc, false);  
            }  
...
$("#stopgamebutton").hide();
    ...
    for (var i = 0; i < 4; i++){
	 ...
	 $('#peiyu').text(getNumberText(max) );
	 $('#peiyu2').text(getNumberText(max) );
	$.toast("游戏结束", "cancel");
	upup(parseInt($('#score').text()));
