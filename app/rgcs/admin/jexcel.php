...
header("Content-Disposition:attachment;filename=choujiang.xls");
$sql = "SELECT * FROM  rg_cj order by addtime desc";
$row = $DB->fetch_all_array($sql);
echo(iconv('UTF-8', 'GBK',  "序号"))."\t"; 
echo(iconv('UTF-8', 'GBK',  "编号"))."\t";
echo(iconv('UTF-8', 'GBK',  "昵称"))."\t"; 
echo(iconv('UTF-8', 'GBK',  "登记名"))."\t";
echo(iconv('UTF-8', 'GBK',  "手机"))."\t";
echo(iconv('UTF-8', 'GBK',  "奖品"))."\t";
 echo(iconv('UTF-8', 'GBK',  "登记时间"))."\t"; 
...
echo   $loopChild['name']."\t"; 
echo   $loopChild['mobile']."\t"; 
echo   $loopChild['cj']."\t"; 
echo  date('Y-m-j G:i:s',$loopChild['addtime'])."\t"; 
echo  date('Y-m-j G:i:s',$loopChild['stime'])."\t"; 
echo   $loopChild['ip']."\t"; 
