...
//开始是否符合要求
$sq = "SELECT * FROM  rg_result WHERE openid = '".$openid."' and sroce='100' order by addtime ASC LIMIT 1";
...
$jp2="";
		...
		$sq3 = "SELECT * FROM  rg_cj WHERE openid = '".$openid."' order by addtime ASC LIMIT 1";
$result3 =$DB->once_fetch_array($sq3);
 if(empty($result3))printjson("no",'未登记');	
  ...
  $Arr = array(
		...
		$row = $DB->updateArr('rg_cj',$Arr,"WHERE id ='{$result3['id']}'");
		if($row){
			printjson("ok",'OK');
		}else{
			printjson("no",'数据库异常');	
