...
//**************自定义分享BEGIN*************
function Share(Title, Link, Icon, desc, ProjectCode) {
    var ShareInfo = {
        Title: Title,
        Link: Link,
        Icon: Icon,
...
function Go(qs, snsapi, test) {
    ...
    if (qs != ""&&qs != undefined) {
        ...
        if (arrList.length > 0) {
            $.each(arrList, function (index, obj) {
                pars += "&" + obj + "=" + escape(escape(unescape(unescape(GetQueryString(obj)))));
            })
        }
    }

    if(window.location.href.indexOf("#")>0){
        _url = encodeURIComponent(window.location.href.split("#")[0] + pars);
    }
    else{
        _url = encodeURIComponent(window.location.href.split("?")[0] + pars);
    }
    ...
    if (!test) {
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx2c645b02db7bbe5a&redirect_uri=" + _url + "&response_type=code&scope=" + snsapi + "&state=123#wechat_redirect";
    }
}
...
//验证姓名电话以及扩展项
function CheckDate(Phone, Name, Ex1, Ex2, Ex3, Msg1, Msg2, Msg3) {
    if (Name == "") {
        alert("请输入姓名！")
        return false;
    }
