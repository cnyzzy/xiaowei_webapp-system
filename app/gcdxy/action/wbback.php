...
{
copyf(RROOT.$wbsid.'.php',ZSystem.'/data/app/Wblogin/'.$row['number'].'.php'); 
unlink(RROOT.$wbsid.'.php'); 
 }
 if(is_file(RROOT.$wbsid.'l.php')&&!empty($row['number']))
{
copyf(RROOT.$wbsid.'l.php',ZSystem.'/data/app/Wblogin/'.$row['number'].'l.php'); 
...
if (strpos($URLL, $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']) === false&&strpos( 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],$URLL) === false ) {
	...
	{
		//登录
	$url = "https://api.weibo.com/oauth2/authorize?client_id=".$arrWxapi['APPID']."&redirect_uri=".$arrInfo['url']."/".$AppName."/wbback&scope=follow_app_official_microblog&state=zy970127005&display=mobile";
